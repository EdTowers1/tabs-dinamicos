...
function MsgInfo(cMsg, fCallback, cTitle, cIcon) {
  ...
  if (typeof cMsg !== 'string' || (typeof cMsg == 'string' && cMsg.length == 0)) cMsg = String(cMsg)
...
function MsgError(cMsg, fCallback, cTitle, cIcon) {
  ...
  if (typeof cMsg !== 'string' || (typeof cMsg == 'string' && cMsg.length == 0)) cMsg = '&nbsp;'
...
function MsgGet(cInput, fCallback, cTitle, cIcon) {
  ...
  if (typeof cInput !== 'string' || (typeof cInput == 'string' && cInput.length == 0)) cInput = String(cInput)
  ...
  var dialog = bootbox.prompt({
    ...
    className: 'animated fadeIn',
    callback: function (result) {
      if (typeof result == 'string') fCallback.apply(null, [result])
    }
  })
...
function MsgLoading(cMessage, cTitle, cIcon, lHeader) {
  ...
  if (!lHeader) _TWebMsgLoading.find('div.modal-header').addClass('loading_header')
...
var TWebGetAutocomplete = function (cId, cSource, bSelect, oConfig, oParameters) {
  ...
  this.Init = function () {
    ...
    if (this.cId_Dialog !== '') $('#' + this.cId).autocomplete('option', 'appendTo', '#' + this.cId_Dialog) //	IMPORTANTE si se ejecuta desde un Dialog
  }

  function TWeb_TGet_Search() {
    if (oGet.trigger === '') return true

    var cValue = this.value

    if (cValue.length < this.minLength) return false
...
function _SidebarInit() {
  ...
  $(document).mouseup(function (e) {
    ...
    if (!_sb.is(e.target) && _sb.has(e.target).length === 0) {
      // Only auto-close the sidebar on small screens (keep it open on large/desktops)
      if (_sbopened) {
        if (window.innerWidth < 992) {
          _sb.trigger('sidebar:close')
          // on larger screens do nothing so clicks on body won't close the sidebar
        }
      }
    }
  })
